...
void recharge_path_tree(struct Path *path) {
  struct Path *aux = path;
  while (aux && 0 == --(aux->colgados)) {
    aux = aux->from;
    path->from = nullptr;
    free(path);
...
void *mov(void *_args) {
  ...
  /*if (0 == is_legal_position(args->pos))
    return nullptr;*/
...
#define call_mov_arg(where_arg, TO, _path, arg, _ret)                          \
  {                                                                            \
    ...
    if (is_legal_position(where_arg) &&                                        \
        ...
        steps_map[map_size_x * map_size_y * (where_arg.orientation) +          \
                  where_arg.x * map_size_x + where_arg.y] > aux.steps) {       \
      steps_map[map_size_x * map_size_y * (where_arg.orientation) +            \
                where_arg.x * map_size_x + where_arg.y] = aux.steps;           \
      _path = malloc(sizeof(struct Path));                                     \
      if (nullptr == _path) {                                                  \
        perror("malloc");                                                      \
        exit(1);                                                               \
      }                                                                        \
